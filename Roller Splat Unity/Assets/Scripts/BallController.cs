...
public class BallController : MonoBehaviour
{
    ...
    // public particle variable
    public ParticleSystem dirtParticle;

    //public audioclip variable
    public AudioClip bumpSound;

    //Private audio source variables
    ...
    private void FixedUpdate()
    {
        ...
        if (nextCollisionPosition != Vector3.zero)
        {
            if (Vector3.Distance(transform.position, nextCollisionPosition) < 1)
            {
                ...
                dirtParticle.Play();
                // To spawn particle upon collision
                Instantiate(dirtParticle, transform.position, dirtParticle.transform.rotation);

                //To play sound effect upon collision
